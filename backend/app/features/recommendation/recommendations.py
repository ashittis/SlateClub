...
from app.shared.models.user import User, UserTasteState
from app.shared.models.movie import Movie
from app.shared.models.actions import Rating, WatchHistory, WatchlistItem
from app.shared.models.social import Follow, MicroFeedback
from app.ml.llm.contextual_bandit import bandit
from app.ml.llm.drift_detector import compute_drift_score
...
from app.ml.embeddings.taste_vector import movie_to_embedding, seed_prior_vector, rating_signal
...
async def for_you(
    ...
    # 4d. Contextual-bandit source blending — scale each source's contribution
    #     by its learned reward for this user's segment. Consumption only; the
    #     reward loop lives in services/watch_signals. No-op with no learned data.
    source_weights = None
    segment = "default"
    try:
        follow_count = (
        ...
        days_since_signup = max((datetime.now(timezone.utc) - user.created_at).days, 0)
        segment = bandit.classify_user(len(ratings), follow_count, days_since_signup)
        source_weights = bandit.get_blend_weights(segment)
    except Exception as exc:
        print(f"[for-you] bandit weights failed: {exc}")

    # 4e. Drift adaptation — a taste phase-transition widens exploration (0.5) and
    #     accelerates decay for THIS request only. Thresholds unchanged.
    explore_override = None
    accel_decay = False
    try:
        drift = compute_drift_score(interactions)
        if drift.get("phase_transition"):
            explore_override = 0.5
            accel_decay = True
    except Exception as exc:
        print(f"[for-you] drift check failed: {exc}")
    ...
    results = pipeline.run(
        ...
        explore_ratio_override=explore_override,
        accelerated_decay=accel_decay,
    )
    ...
    return {
        ...
        "pipeline": "ml-v1",
        # Diagnostics: which bandit segment drove source blending, and whether a
        # drift phase-transition widened exploration this request.
        "debug": {
            "segment": segment,
            "source_weights": source_weights,
            "explore_ratio": explore_override if explore_override is not None else 0.3,
            "drift_adapted": accel_decay,
        },
    }
