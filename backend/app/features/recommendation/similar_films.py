...
# We always build one broad pool of this many films per seed, cache it, and
# serve tight 5-film windows out of it (see answer_slice). Bump _ESSENCE_VERSION
        ...
        "films": {
            ...
            "items": {
                ...
                "properties": {
                    ...
                    "why": {
                        "type": "string",
                        "description": "One plain-English line, max 12 words, naming "
                        "the shared FEELING — not the plot. This is the headline of "
                        "the answer, so make it land. e.g. 'ambition curdles into "
                        "self-destruction' or 'class satire that turns to dread'.",
                    },
                },
                "required": ["title", "why"],
            },
...
def _essence_prompt(seed: Movie, community_text: str = "") -> str:
    ...
    return (
        ...
        "STRONGEST match first — the top of the list is what the viewer actually "
        "sees. PRIORITISE emotional arc and aftertaste over shared "
        "profession/genre/setting: a film about a totally different subject that "
        "delivers the same felt journey beats a same-profession film that feels "
        "different. But keep TONE true — do NOT lump a melancholic, heartbreaking "
        "film together with a defiant, cheer-out-loud one just because they share "
        "an arc shape. Language is NOT a barrier: draw from ANY industry worldwide "
        "— Hollywood (English), Bollywood (Hindi), Kollywood (Tamil), Tollywood "
        "(Telugu), Mollywood (Malayalam), Sandalwood (Kannada), plus Korean, "
        "Japanese, European and others, spread across languages. Exclude the seed "
        "itself. For each, give its real title, release year, and one "
        "plain-English line (max 12 words) naming the shared feeling — not the plot."
    )
...
async def _essence_films(
    ...
    language so the client can filter/window without another LLM call. Returns
...
async def find_similar_films(
    ...
    Makes ONE LLM call returning a broad, language-diverse set of ~30 films
    ranked closest-feel first, each tagged with its original language, plus a
    director/cast row. Falls back to cosine ranking when the LLM is unavailable.
    Cached per seed in memory + DB (SimilarCache, keyed by tmdb_id — films only,
    since a movie and series can share a tmdb_id).

    Callers serve tight windows out of `results` via `answer_slice`; this
    function never slices, so one cached pool answers every offset + language
    filter without another LLM/TMDB round-trip.
    """
    ...
    if results is None:
        ...
        results = _cosine_similar_films(db, seed, _POOL_SIZE, await _all_movies(db))
...
def answer_slice(
    payload: dict, *, offset: int = 0, languages: list[str] | None = None
) -> dict:
    """Serve a tight window out of a cached pool — the actual 4–5-film answer.

    Filters by original language (server-side, so the client never holds 30),
    slices `[offset : offset+5]`, and only attaches the creator/cast row on the
    first, unfiltered view (it's a footer to the whole answer, not each page).
    Pure — no I/O — so it's cheap on every cache hit and unit-testable."""
    results = list(payload.get("results") or [])
    if languages:
        wanted = {lang.lower() for lang in languages if lang}
        if wanted:
            results = [
                r for r in results
                if (r.get("originalLanguage") or "").lower() in wanted
            ]

    pool_size = len(results)
    window = results[offset : offset + _ANSWER_SIZE]
    first_view = offset == 0 and not languages

    return {
        "seed": payload.get("seed"),
        "essence": payload.get("essence"),
        "answer": window,
        "creatorRow": payload.get("creatorRow") if first_view else None,
        "poolSize": pool_size,
        "offset": offset,
        "hasMore": offset + _ANSWER_SIZE < pool_size,
    }
