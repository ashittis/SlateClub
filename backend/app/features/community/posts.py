...
from app.features.community.models.posts import POST_TYPES, Post, PostReply, PostUpvote
...
async def list_posts(
    ...
    # Upvote tallies for ranking. Left-joined so zero-upvote posts still appear.
    upvote_sq = (
        select(PostUpvote.post_id, func.count().label("cnt"))
        .group_by(PostUpvote.post_id)
        .subquery()
    )
    score = func.coalesce(upvote_sq.c.cnt, 0)

    stmt = (
        select(Post, User)
        .join(User, User.id == Post.user_id)
        .outerjoin(upvote_sq, upvote_sq.c.post_id == Post.id)
        .limit(limit)
        .offset(offset)
    )

    if sort == "top":
        stmt = stmt.order_by(desc(score), desc(Post.created_at))
    elif sort == "hot":
        # Top posts from the last week — score with recency as tiebreaker.
        cutoff = datetime.now(timezone.utc) - timedelta(days=7)
        stmt = stmt.where(Post.created_at >= cutoff).order_by(
            desc(score), desc(Post.created_at)
        )
    else:  # new
