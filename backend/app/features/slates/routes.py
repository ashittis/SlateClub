...
from app.features.slates.models import (
    SLATE_COLLAB_ROLES,
    SLATE_COVER_LAYOUTS,
    SLATE_VISIBILITIES,
    Slate,
...
async def _enrich_films(
    ...
    # Self-heal: films added by tmdb id that were never cached into `movies`
    # render as bare "#<id>" because title/poster resolve to None. Lazily fetch
    # + cache the missing ones so they show properly (and stay cached after).
    missing = [f for f in films if (f.tmdb_id, f.media_type) not in by_key]
    if missing:
        from app.features.movies.movies import _get_or_fetch_movie

        for f in missing:
            if (f.tmdb_id, f.media_type) in by_key:
                continue
            try:
                m = await _get_or_fetch_movie(f.tmdb_id, db, f.media_type)
            except HTTPException:
                continue  # unknown id — leave it as a bare reference
            by_key[(m.tmdb_id, m.media_type)] = m
...
async def add_film(
    ...
    # Cache the movie so its title/poster resolve in the slate view. Best-effort:
    # adding still succeeds even if TMDB is unreachable (enrich retries on read).
    from app.features.movies.movies import _get_or_fetch_movie

    try:
        await _get_or_fetch_movie(body.tmdb_id, db, body.media_type)
    except HTTPException:
        pass
