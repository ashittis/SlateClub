...
async def _enrich_films(
    ...
    by_id = {m.tmdb_id: m for m in movie_rows}
    out = []
    for f in films:
        m = by_id.get(f.tmdb_id)
        out.append(
            {
                "tmdbId": f.tmdb_id,
                "note": f.note,
...
async def _slate_card(slate: Slate, db: AsyncSession) -> dict:
...
async def create_slate(
    ...
    db.add(slate)
    await db.flush()
    return await _slate_card(slate, db)
...
async def get_slate(
    ...
    if slate.visibility == "private" and (not user or user.id != slate.creator_id):
        raise HTTPException(status_code=404, detail="Slate not found")
