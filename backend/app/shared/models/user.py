...
class User(Base):
    ...
    # IP-derived location, refreshed on visit (source of truth is the request
    # IP, not user input). Powers the "Trending in your city" home rail.
    city: Mapped[str | None] = mapped_column(String, nullable=True)
    region: Mapped[str | None] = mapped_column(String, nullable=True)
    country: Mapped[str | None] = mapped_column(String, nullable=True)
    geo_updated_at: Mapped[datetime | None] = mapped_column(
        "geoUpdatedAt", DateTime(timezone=True), nullable=True
    )
